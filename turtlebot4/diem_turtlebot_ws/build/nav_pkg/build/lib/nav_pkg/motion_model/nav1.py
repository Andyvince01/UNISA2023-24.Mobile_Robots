...
# LINEAR_SPEED = 0.25         # Linear speed in m/s (maximal speed: 0.31 m/s)
# ANGULAR_SPEED = np.pi/2     # Angular speed in rad/s (maximal speed: 1.5 rad/s)
...
class Handler(Node):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.declare_parameter('std_dev', 0)
    ...
    def loop(self) -> None:
        ...
        current_pose : Pose2 = self.landmarks[0]
        ...
        for target_pose in self.landmarks[1:]:
            ...
            target_pose = Pose2.add_gaussian_noise(pose=target_pose, mean=self.mean, std_dev=self.std_dev)
            ...
            angle_rot1 = np.arctan2(target_pose.position.y - current_pose.position.y, target_pose.position.x - current_pose.position.x) - current_pose.orientation.yaw
            self._rotate(angle_rot1)
            
            # Moving the robot to the target pose
            distance : float = Pose2.distance(current_pose, target_pose)
            self._move(distance)
            
            # Second Rotation of the robot to the target pose
            angle_rot2 = target_pose.orientation.yaw - current_pose.orientation.yaw
    ...
    def _read_landmarks(self, file_path: str, verbose: bool = False) -> list:
        ...
        with open(file_path, 'r') as file:
            ...
            for key, value in poses.items():
                ...
                landmarks.append(Pose2(id=key, position=pos, orientation=ori))
